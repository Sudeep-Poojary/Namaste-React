...
Ans: Short form for `properties`. To `dynamically` send data to a component we use props. Passing a prop to a function is like passing an `argument` to a `function`.
...
Ans: It is a user Interface that is built and configured using a `declaration` configuration file or `data structure`, rather than being hardcoded. Config is the data coming from the `API` which keeps on changing according to different factors like user, location, etc.
